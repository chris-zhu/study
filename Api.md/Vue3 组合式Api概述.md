...
2. 目前缺少一种简洁且低成本的机制来提取和重用多个组件之间的逻辑。 (详见 [逻辑抽象、提取与复用](https://composition-api.vuejs.org/zh/#逻辑抽象、提取与复用))

RFC 中提出的 API 为组件代码的组织提供了更大的灵活性。现在我们不需要总是通过选项来组织代码，而是可以将代码组织为处理特定功能的函数。这些 API 还使得在组件之间甚至组件之外逻辑的提取和重用变得更加简单。我们会在[设计细节](https://composition-api.vuejs.org/zh/#设计细节)这一节展示达成的效果。
...
请注意，本节主要会介绍这些 API 的基本思路，所以不会展开至其完整的细节。完整的 API 规范请移步 [API 参考](https://composition-api.vuejs.org/zh/api)章节。
...
>
> 但只是为了在模板中获取 DOM 元素或组件实例 (“模板引用”)。可以到[这里](https://composition-api.vuejs.org/zh/api.html#template-refs)查看新的 ref 系统如何同时用于逻辑状态和模板引用。
...
> 关于这些 API 的更多细节可以在 [API 参考](https://composition-api.vuejs.org/zh/api)页面中找到。但是在深入挖掘设计细节之前，我们建议你先将接下来的章节看完。
...
![file explorer (before)](https://user-images.githubusercontent.com/499550/62783021-7ce24400-ba89-11e9-9dd3-36f4f6b1fae2.png)
...
当使用组合式 API 时，我们不再使用 `this`，取而代之的是，插件将在内部利用 [`provide` 和 `inject`](https://composition-api.vuejs.org/zh/api.html#依赖注入) 并暴露一个组合函数。以下是一个插件的假设代码：
...
2. 读写 ref 的操作比普通值的更冗余，因为需要访问 `.value`。
   ...
   有人建议使用编译时的语法糖 (类似 Svelte 3) 来解决这个问题。这虽然在技术上可行，但我们并不认为 Vue 需要默认支持 (正如在[与 Svelte 比较](https://composition-api.vuejs.org/zh/#与-svelte-比较)中所讨论的那样)。换而言之，可以在用户端开发一个 Babel 插件来处理。
...
[`toRefs`](https://composition-api.vuejs.org/zh/api.html#torefs) API 用来提供解决此约束的办法——它将响应式对象的每个 property 都转成了相应的 ref。
